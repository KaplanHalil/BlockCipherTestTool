...
def process_bit_mk_rk_corr(bit_index):
    """Process a single bit index for mk-rk correlation test"""
    ...
    for k in range(1000):
        ...
        for m in range(len(rkeys_bits)):
            for n in range(len(rkeys_bits[0])):
                if rkeys_bits[m][n] == mkey_bits[bit_index]:
                    result[m][n] += 1
...
if __name__ == "__main__":
    ...
    with Pool(processes=num_workers) as pool:
        results = pool.map(process_bit_mk_rk_corr, bit_indices)
    ...
    # Draw vertical lines to distinguish round keys
    draw_round_key_lines(img)

    img.save(os.path.join(results_dir, "corr_mk-rk.png"))
    b=time.time()
