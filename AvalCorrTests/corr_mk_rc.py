...
def process_bit_mk_rc_corr(bit_index):
    """Process a single bit index for mk-rc correlation test"""
    ...
    for k in range(1000):
        ...
        for m in range(len(ciphertext_bits)):
            for n in range(len(ciphertext_bits[0])):
                if ciphertext_bits[m][n] == mkey_bits[bit_index]:
                    result[m][n] += 1
...
if __name__ == "__main__":
    ...
    with Pool(processes=num_workers) as pool:
        results = pool.map(process_bit_mk_rc_corr, bit_indices)
    ...
    img.save(os.path.join(results_dir, "corr_mk-rc.png"))
    b=time.time()
